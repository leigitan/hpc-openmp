...
vector<pair<int, int>> getPruferEdges(const vector<int>& P, int n) {
    ...
    // Поки покажчик не дійшов до кінця масиву
    while (p_index < p_size) {
        ...
        for (int x : V) {
            bool found = false;
            // Шукаємо 'x' тільки в частині вектора, що залишилася (непрочитаної) P
        ...
        edges.push_back({u, v});
        p_index++; // Зрушуємо покажчик вперед замість P.erase()
        V.erase(v);
    }
    ...
    for (int step = 0; step < n; ++step) {
        for (auto& edge : edges) {
            ...
            if (visited[u] && !visited[v]) {
                weights[v] = weights[u] / mpc.data[u][v]; // Транзитивність
                visited[v] = true;
...
vector<ld> calculatePriorityVector(const MatrixPC& mpc, bool useGeometric, int& validTreesCount) {
    ...
    // Зводимо n у ступінь (n - 2) виключно в цілих числах
    for (int i = 0; i < n - 2; ++i) {
        total_combinations *= n;
    }

    // Вектор накопичення результатів
    // Для арифметичного середнього накопичуємо суму: Σw
    // Для геометричного середнього накопичуємо суму логарифмів: ∑ln(w)
    vector<ld> accumulated_weights(n, 0.0);
    validTreesCount = 0;
    ...
    // Фінальне усереднення
    vector<ld> final_vector(n);
    if (validTreesCount > 0) {
        ld sum_for_normalization = 0;
        for (int j = 0; j < n; ++j) {
            if (useGeometric) {
                // Середнє геометричне через експоненту: exp((1/k) * Σln(w) )
...
int main() {
    ...
    cout << "\nЧас виконання (послідовно): " << duration.count() << " сек." << endl;
